...
### Book
- 역할/책임 : 책 한권의 메타데이터를 관리하고 여러 소장본의 상위 개념으로서 예약의 기준이 된다.
- 행동 :
...
- 행동 : 
  - 대출 가능일 때만 대출로 전환한다.
  - 반납 시 예약 대기열이 없으면 다시 대출 가능으로 상태를 갱신한다.
  - 반납 시 예약 대기열이 있으면 지정 회원의 픽업 대기로 전환된다.
...
### Reservation
- 역할/책임 : `Book` 단위의 예약 대기열을 관리하며 반납 등으로 가용 소장본이 생길 때 선두 회원에게 우선권을 부여한다.
- 행동 :
  - 동일 회원의 중복 예약을 금지한다.
  - 대기열 상한을 넘기면 예약을 거절한다.
  - 선두에게 우선권이 부여되면 특정 `StoredBook`을 `ON_HOLD`로 변경한다.
  - 기한 내 미수령 시 다음 대기자에게 순번을 넘긴다.
